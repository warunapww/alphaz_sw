...
void sw_verify(long OM, long ON, long ObM, long ObN, short** Oseq_A, short** Oseq_B, int** OAout, int** OBout) {
//		long M, long N, short* _local_seq_A, short* _local_seq_B, int* _local_A, int* _local_B, int* _local_c, int* _local_Aout, int* _local_Bout){
	...
	short* _local_seq_A = (short *)malloc(sizeof(short)*M);
	short* _local_seq_B = (short *)malloc(sizeof(short)*N);

	int* _local_A = (int *) malloc(sizeof(int)*M);
	int* _local_B = (int *) malloc(sizeof(int)*N);
	int _local_c = 0;
	int* _local_Aout = (int *) malloc(sizeof(int)*M);
	int* _local_Bout = (int *) malloc(sizeof(int)*N);

	memset(_local_A,0,sizeof(int)*M);
	memset(_local_B,0,sizeof(int)*N);

	int ii=0;
	int jj=0;

	for (ii=0;ii<OM; ii++) {
		memcpy(&_local_seq_A[ii*ObM],Oseq_A[ii],sizeof(short)*ObM);
	}
	for (ii=0;ii<ON; ii++) {
		memcpy(&_local_seq_B[ii*ObN],Oseq_B[ii],sizeof(short)*ObN);
	}
	...
	for (ii=0;ii<OM; ii++) {
		memcpy(OAout[ii],&_local_Aout[ii*ObM],sizeof(int)*ObM);
	}
	for (ii=0;ii<ON; ii++) {
		memcpy(OBout[ii],&_local_Bout[ii*ObN],sizeof(int)*ObN);
	}
